...
export const DropdownMode: React.FC<DropdownModeProps> = ({ selectedMode, onChange }) => {
  ...
  return (
    <div className='relative' ref={ref}>
      ...
      {isOpen && (
        <div
          className='w-[200px] absolute top-8 left-0 mt-2 rounded-b-2xl shadow-lg bg-yellow-400 ring-1 ring-black ring-opacity-5'
          role='menu'
          ...
          <div className='py-1'>
            {Object.values(Mode).map((option) => (
              <button
                ...
                className='block w-full text-left px-4 py-2 text-sm hover:bg-yellow-300 hover:rounded-2xl'
              >
                {option}
              </button>
            ))}
